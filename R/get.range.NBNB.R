...
# - "mlci": approximate marginal 95% likelihood confidence intervals
   ...
   if (method=="best") {
      ...
      ranges <- calculate.ci(alpha=0.05,parameter=best,dataset=dataset,n=n,TY=TY)
      return(ranges)
   }
   ...
   if (method!="best") {
      ...
      #size and mu together
      ranges[TY:nrow(ranges),1] <- pmax(ranges[TY:nrow(ranges),1],0.01)
      ranges[TY:nrow(ranges),2] <- pmax(ranges[TY:nrow(ranges),2],ranges[TY:nrow(ranges),1]+0.05)
   }
