...
# - The size-values for the first gene are decreasing.
#
# m is the number of genes analyzed.


   # number of types
   TY <- (nrow(par.range)+1)/(2*m+1)
   ...
   ###################################################
   # sort the size values and p values and mu values #
   ...
   # size
   draw[TY:((m+1)*TY-1)] <- c(t(size))

   # mu
   draw[((m+1)*TY):length(draw)] <- c(t(mu))
   return(draw)
}
