...
# with probability p_i, a summand is of type i. In that case, it is negative binomially
# distributed with size size_i and mu mu_i.
#
# Parameters:
#
...
# Their lengths automatically determine the number of different types.
   ...
   for (j in 1:k) {
      # j.th sample
      N.vector <- N.matrix[j,]
      # expand size.vector and mu.vector
      full.size.vector <- rep(0,n[j])
      full.mu.vector <- full.size.vector
      index <- 0
      for (i in 1:length(p.vector)) {
         if (index<n[j]) {
            full.size.vector[index+(1:N.vector[i])] <- size.vector[i]
            full.mu.vector[index+(1:N.vector[i])] <- mu.vector[i]
            index <- index + N.vector[i]
         }
      }
      # vector of summands
      Y <- rnbinom(n[j],size = full.size.vector,mu = full.mu.vector)
      # sum up
      random[j] <- sum(Y)
   }
