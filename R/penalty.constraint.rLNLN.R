...
function(dataset,parameter,smoothingpar=10^5) {
   ...
   m <- ncol(dataset)
   TY <- (length(parameter)+1)/(m+2)
   mu <- parameter[TY:((m+1)*TY-1)]
   sigma <- parameter[(m+1)*TY+(0:(TY-1))]
   ...
   for (g in 1:m) {
      ...
      for (i in 1:(TY-1)) {
         ...
         if (mode.ig<max.datapoint) {
            ...
            mode.igplus1 <- exp(mu[i+1,g]-sigma[i+1]^2)
            if (mode.igplus1>mode.ig) {
               x <- c(x,mode.igplus1)
               x <- x[order(x)]
            }            
            # density of lognormal i at these values
            d.LN1 <- d.sum.of.lognormals(y=x,mu.vector=mu[i,g],sigma.vector=sigma[i])
            # density of lognormal i+1 at these values
            d.LN2 <- d.sum.of.lognormals(y=x,mu.vector=mu[i+1,g],sigma.vector=sigma[i+1])
            ...
            if (sum(d.LN2)==0) {
               if (mode.igplus1>mode.ig) {
                  pen <- pen + max(0,dlnorm(mode.igplus1,mu[i+1,g],sigma[i+1])-dlnorm(mode.igplus1,mu[i,g],sigma[i]))^2
               }
            }            
         }
      }
